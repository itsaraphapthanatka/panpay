...
function Plans({ plans, reload, onError }) {
              ...
              <td style={{ textAlign: "right" }}>
                {p.active && (
                  <button className="btn ghost" style={{ padding: "4px 10px", marginRight: 6 }}
                          onClick={() => api.updatePlan(p.id, { active: false }).then(reload).catch((e) => onError(e.message))}>
                    ปิดใช้งาน
                  </button>
                )}
                <button className="btn danger" style={{ padding: "4px 10px" }}
                        onClick={() => api.deletePlan(p.id).then(reload).catch((e) => onError(e.message))}>
                  ลบ
                </button>
              </td>
            </tr>
...
function NewMember({ plans, reload, onError, onInvoice }) {
  ...
  return (
    <form onSubmit={add} style={{ display: "flex", gap: 8, alignItems: "flex-end", flexWrap: "wrap" }}>
      ...
      <label className="field" style={{ marginBottom: 0, width: 110 }}>
        ...
        <input value={form.coupon_code} onChange={(e) => setForm({ ...form, coupon_code: e.target.value })} placeholder="ไม่บังคับ" />
      </label>
...
function Coupons({ coupons, reload, onError }) {
          ...
          {coupons.map((c) => (
            <tr key={c.id}>
              ...
              <td style={{ textAlign: "right" }}>
                {c.active && (
                  <button className="btn danger" style={{ padding: "4px 10px" }}
                          onClick={() => api.deleteCoupon(c.id).then(reload).catch((e) => onError(e.message))}>
                    ปิด
                  </button>
                )}
              </td>
...
export default function Members() {
  ...
  const [changing, setChanging] = useState(null); // { sub, planId }

  async function load() {
    try {
      const [p, s, st, c, n] = await Promise.all([
        api.listPlans(), api.listSubscriptions(), api.subscriptionStats(),
        api.listCoupons(), api.listNotifications(),
      ]);
      setPlans(p);
      setSubs(s);
      setStats(st);
  ...
  async function renew(id) {
    try {
      const inv = await api.renewSubscription(id);
      setInvoice(inv);
      load();
    } catch (e) {
      setErr(e.message);
  ...
  return (
    <div>
      ...
      <p className="page-sub">แผนสมาชิกแบบจ่ายตามรอบผ่าน PromptPay — จ่ายแล้วต่ออายุอัตโนมัติ</p>
      ...
      <div className="grid cols-2" style={{ marginBottom: 16 }}>
        <Plans plans={plans} reload={load} onError={setErr} />
        <Coupons coupons={coupons} reload={load} onError={setErr} />
      </div>
      ...
      <div className="card">
        ...
        <NewMember plans={plans} reload={load} onError={setErr} onInvoice={setInvoice} />
