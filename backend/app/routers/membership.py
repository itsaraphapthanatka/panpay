...
from ..models import Charge, Coupon, Merchant, NotificationLog, Plan, Subscription, utcnow
...
router = APIRouter(prefix="/dashboard", tags=["membership"])


# ---- Plans ----
@router.get("/plans", response_model=list[PlanOut])
def list_plans(merchant: Merchant = Depends(get_current_merchant), db: Session = Depends(get_db)):
    return (
...
def create_plan(
    ...
    db.commit()
    db.refresh(plan)
    audit.record(db, action="plan.create", actor=merchant.email, merchant_id=merchant.id,
                 target_type="plan", target_id=plan.id, request=request)
    return plan
...
def _get_plan(db, plan_id, merchant) -> Plan:
    plan = db.get(Plan, plan_id)
    if not plan or plan.merchant_id != merchant.id:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Plan not found")
    return plan
...
def _get_sub(db, sub_id, merchant) -> Subscription:
...
def stats(merchant: Merchant = Depends(get_current_merchant), db: Session = Depends(get_db)):
...
def create_sub(
    ...
    audit.record(db, action="subscription.create", actor=merchant.email, merchant_id=merchant.id,
...
def change_sub_plan(
    ...
    audit.record(db, action="subscription.change_plan", actor=merchant.email, merchant_id=merchant.id,
                 target_type="subscription", target_id=sub.id, request=request,
                 extra={"plan_id": new_plan.id})
    invoices = (
...
def get_sub(sub_id: str, merchant: Merchant = Depends(get_current_merchant), db: Session = Depends(get_db)):
    sub = _get_sub(db, sub_id, merchant)
    invoices = (
        db.query(Charge)
...
def renew(sub_id: str, merchant: Merchant = Depends(get_current_merchant), db: Session = Depends(get_db)):
    sub = _get_sub(db, sub_id, merchant)
...
def cancel(
    ...
    audit.record(db, action="subscription.cancel", actor=merchant.email, merchant_id=merchant.id,
                 target_type="subscription", target_id=sub.id, request=request)
...
def generate_due(
    ...
    audit.record(db, action="subscription.generate_due", actor=merchant.email, merchant_id=merchant.id,
                 request=request, extra={"count": len(invoices)})
    return [charge_to_out(c) for c in invoices]


# ---- Coupons ----
@router.get("/coupons", response_model=list[CouponOut])
def list_coupons(merchant: Merchant = Depends(get_current_merchant), db: Session = Depends(get_db)):
    return (
...
def create_coupon(
    ...
    db.commit()
    db.refresh(coupon)
    audit.record(db, action="coupon.create", actor=merchant.email, merchant_id=merchant.id,
                 target_type="coupon", target_id=coupon.id, request=request)
    return coupon
